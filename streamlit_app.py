...
st.set_page_config(
    page_title="Conversa con Todo💬", page_icon="🤗", layout="wide", initial_sidebar_state="expanded"
)
...
with st.sidebar:
    ...
    if 'hf_email' not in st.session_state or 'hf_pass' not in st.session_state:
        with st.expander("ℹ️ Login en Hugging Face", expanded=True):
            st.write("⚠️ Necesitas iniciar sesión en Hugging Face para usar esta aplicación. Puedes registrarte. [here](https://huggingface.co/join).")
            st.header('Hugging Face Login')
            hf_email = st.text_input('Ingrese E-mail:')
            hf_pass = st.text_input('Ingrese password:', type='password')
            hf_token = st.text_input('Ingrese API Token:', type='password')
            if st.button('Login 🚀') and hf_email and hf_pass and hf_token: 
                with st.spinner('🚀 ingresando...'):
                    ...
                    try:
                        ...
                        st.error(e)
                        st.info("⚠️ Por favor, verifica tus credenciales e intenta nuevamente.")
                        st.error("⚠️ no abuses de la API")
                        st.warning("⚠️ Si no tienes una cuenta, puedes registrarte. [here](https://huggingface.co/join).")
                    ...
                    if 'generated' not in st.session_state:
                        st.session_state['generated'] = ["Soy AIcito, como puedo ayudarte? "]
        ...
        with st.expander("ℹ️ Configuracion Avanzada"):
            ...
            repetition_penalty = st.slider('🖌 Penalización por repetición.', min_value=1.0, max_value=2.0, value=1.2, step=0.01)
            top_k = st.slider('❄️ Top K', min_value=1, max_value=100, value=50, step=1)
            max_new_tokens = st.slider('📝 Máximo de tokens nuevos.', min_value=1, max_value=1024, value=1024, step=1)
        ...
        plugins = ["🛑 Sin PLUGIN","🌐 Web Search", "🔗 Talk with Website" , "📋 Talk with your DATA", "📝 Talk with your DOCUMENTS", "🎧 Talk with your AUDIO", "🎥 Talk with YT video", "🧠 GOD MODE" ,"💾 Upload saved VectorStore"]
        if 'plugin' not in st.session_state:
            st.session_state['plugin'] = st.selectbox('🔌 Plugins', plugins, index=0)
        else:
            if st.session_state['plugin'] == "🛑 Sin PLUGIN":
                            ...
                            try:
                                ...
                                with st.spinner(f'👨‍💻 Scraping YT video : {id}'):
                                    ...
                                    if transcript_en is None:   
                                        ...
                                        # transcript_en = transcript_en.translate('es')
...
with input_container:
        input_text = st.chat_input("🧑‍💻 Escribir Aqui 👇", key="input")
